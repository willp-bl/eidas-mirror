...
public class EidasAttributeValidator extends AttributeSchemaValidator {
    ...
    public void validate(Attribute attr) throws ValidationException {
        ...
        if (!attr.getAttributeValues().isEmpty()) {
            ...
            if (value == null) {
                //only isAgeOver can be empty if provided
                if (attrName.equals(eu.stork.peps.auth.engine.core.validator.STORKAttributes.STORK_ATTRIBUTE_ISAGEOVER)) {
                    return;
                } else {
            ...
            if (attrName.equals(eu.stork.peps.auth.engine.core.validator.eidas.EIDASAttributes.ATTRIBUTE_DATEOFBIRTH)) {
                verifyDate(value);
            }

            //STILL perform STORK validation
