...
public final class PEPSUtil {
     ...
     * Logger object.
     */
    private static final Logger LOG = LoggerFactory.getLogger(PEPSUtil.class.getName());
    ...
    /**
     * Contains the package name used to detect wether an error needs to be sent from CPEPS
     */
    private static final String PACKAGE_NAME_CPEPS = "eu.stork.peps.cpeps";
    ...
    public static void setConfigs(final Properties nConfigs) {
        if (nConfigs == null) {
            PEPSUtil.configs = loadConfigs("pepsUtil.properties");
        }else {
            PEPSUtil.configs = nConfigs;
        }
    }
    ...
    public static void validateParameter(final String className,
        ...
        if (!isValidParameter(paramName, paramValue)) {
            ...
            if (className !=null && className.startsWith(PACKAGE_NAME_CPEPS)){
                throw new InvalidParameterCPEPSException(errorCode, errorMessage);
            } else {
    ...
    public static String encodeSAMLToken(final byte[] samlToken) {
        try {
            ...
            LOG.info(PEPSErrors.INTERNAL_ERROR.errorMessage(), e);
            return null;
        }
    }
    ...
    public static byte[] hashPersonalToken(final byte[] samlToken) {
        String className = PEPSUtil.getConfig(PEPSValues.HASH_DIGEST_CLASS.toString());
    ...
    public static byte[] hashPersonalToken(final byte[] samlToken, String className) {
        ...
        try {
            ...
            LOG.info(PEPSErrors.HASH_ERROR.errorMessage(), e);
    ...
    public static Map<String, String> getHttpRequestParameters(String className, HttpServletRequest request) {
        ...
        if (request.getHeader(PEPSParameters.HTTP_X_FORWARDED_FOR.toString()) == null) {
            if (request.getHeader(PEPSParameters.X_FORWARDED_FOR.toString()) != null) {
                remoteAddr = request.getHeader(PEPSParameters.X_FORWARDED_FOR.toString());
            }
        } else {
            remoteAddr = request.getHeader(PEPSParameters.HTTP_X_FORWARDED_FOR.toString());
        }

        final String remoteAddrCons = PEPSParameters.REMOTE_ADDR.toString();
        PEPSUtil.validateParameter(className, remoteAddrCons, remoteAddr);
        httpParameters.put(remoteAddrCons, remoteAddr);
        httpParameters.put(PEPSParameters.BINDING.toString(), request.getMethod());
    ...
    public static Properties loadConfigs(String path, boolean logError) {
        ...
        InputStream is=null;
        try {
            is=PEPSUtil.class.getClassLoader().getResourceAsStream(path);
            if(is==null) {
            ...
            if(logError) {
                LOG.error("An error occour when trying to load configuration file: " + path, e);
                throw new InternalErrorPEPSException("5", "Internal server error on loading configurations", e);
            }
        }finally{
            if(is!=null){
                try{
                   is.close();
                    ...
                    if(logError) {
                        LOG.error("An error occured when trying to cloase resource stream: ", e);
                    }
                }
            }
        }
    ...
    private static InputStream indirectedProps(String path) throws IOException{
        Properties indirection = PEPSUtil.loadConfigs("configlocation.properties");
        String location=indirection.getProperty(path);
        InputStream is=null;
        if(location!=null){
            if(location.startsWith(CLASSPATH_PREFIX)){
                is=PEPSUtil.class.getClassLoader().getResourceAsStream(location.substring(CLASSPATH_PREFIX.length()));
