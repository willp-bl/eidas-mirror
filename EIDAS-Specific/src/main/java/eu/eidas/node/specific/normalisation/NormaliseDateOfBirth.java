 ...
 */
package eu.eidas.node.specific.normalisation;
...
public final class NormaliseDateOfBirth implements INormaliseValue {
    ...
    */
   public void
      normaliseAttributeValueToFormat(final PersonalAttribute personalAttribute) {
      ...
      if (values != null && values.size() == 1 && values.get(0) != null) {
         ...
         if (hasSeparator) {
            birthdate =
               birthdate.replace(specificSeparator, EIDASValues.EMPTY_STRING
                  .toString());
         }
         ...
         if (!DateUtil.isValidFormatDate(birthdate, pattern)) {
            ...
            throw new SecurityEIDASException(
               EIDASUtil.getConfig(EIDASErrors.INVALID_ATTRIBUTE_VALUE
                  .errorCode()),
               EIDASUtil.getConfig(EIDASErrors.INVALID_ATTRIBUTE_VALUE
