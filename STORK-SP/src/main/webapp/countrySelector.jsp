...
<br />
<iframe
	src="<%=pepsCountryForm%>?spId=<%=spId%>&providerName=<%=providerName%>&spUrl=<%=spUrl%>&spQaaLevel=<%=spQaaLevel%>&attrList=<%=java.net.URLEncoder.encode(attrList, enc)%>&spSector=<%=spSector%>&spInstitution=<%=spInstitution%>&spApplication=<%=spApplication%>&spCountry=<%=spCountry%>&spmetadataurl=<%=spMetadataUrl%>"
