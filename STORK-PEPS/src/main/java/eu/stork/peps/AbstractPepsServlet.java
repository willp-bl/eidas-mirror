...
import eu.stork.peps.logging.LoggingUtil;
...
public abstract class AbstractPepsServlet extends HttpServlet {
     ...
     * @return Peps applicationContext
     */
    ...
    protected final Map<String, String> getHttpRequestParameters(HttpServletRequest request) {
        return PEPSUtil.getHttpRequestParameters(this.getClass().getCanonicalName(), request);
...
//NOSONAR                 response.addCookie(cookie);
                        ...
                        StringBuilder httpOnlyCookie = new StringBuilder(cookie.getName()).append(PEPSValues.EQUAL.toString()).append(cookie.getValue()).append(PEPSValues.SEMICOLON.toString()).append(" ")
                                .append(PEPSValues.DOMAIN.toString()).append(PEPSValues.EQUAL.toString()).append(request.getServerName()).append(PEPSValues.SEMICOLON.toString()).append(" ")
                                .append(PEPSValues.PATH.toString()).append(PEPSValues.EQUAL.toString()).append(getServletContext().getContextPath()).append(PEPSValues.SEMICOLON.toString()).append(" ")
                                .append(PEPSValues.HTTP_ONLY.toString()).append(PEPSValues.SEMICOLON.toString())
                                .append(isSecure ? PEPSValues.SECURE.toString()  : "");
                        response.setHeader(PEPSValues.SETCOOKIE.toString(), httpOnlyCookie.toString());
                    }
                }
            }
        } else {
    ...
    protected final boolean acceptsHttpRedirect(){
        Boolean acceptGet = ApplicationContextProvider.getPepsParameterBool(PEPSParameters.ALLOW_REDIRECT_BINDING.toString());
