...
import eu.stork.peps.auth.commons.CountrySpecificService;
import eu.stork.peps.auth.cpeps.InactiveIntegrationPlugin;
...
public class CountrySpecificUtil implements ApplicationContextAware {
    ...
    private Map<String, Boolean> getConfiguredPlugins(ApplicationContext ctx){
        Properties pepsConfig=(Properties)ctx.getBean("pepsProps");
        Map<String, Boolean> confPlugins=new HashMap<String, Boolean>();
        if(pepsConfig!=null){
            for(String key:pepsConfig.stringPropertyNames()){
                if(key.startsWith(PLUGIN_ACTIVATION_PREFIX)){
                    if(Boolean.valueOf(pepsConfig.getProperty(key))) {
